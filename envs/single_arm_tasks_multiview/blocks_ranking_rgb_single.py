...
class blocks_ranking_rgb_single(Base_Task_Multiview):
    """
    Single-arm: place red, green, and blue blocks in a row from left to right (red, green, blue).
    """
    ...
    def play_once(self):
        # Single arm: choose by first block (red) position
        block1_pose = self.block1.get_pose().p
        arm_tag = ArmTag("left" if block1_pose[0] < 0 else "right")
        self.arm_tag = arm_tag

        # Place red, green, blue in order (left to right)
