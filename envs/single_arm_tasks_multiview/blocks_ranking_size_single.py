...
class blocks_ranking_size_single(Base_Task_Multiview):
    """
    Single-arm: arrange three blocks by size (largest to smallest, left to right) at the center.
    """
    ...
    def play_once(self):
        # Single arm: choose by first block we pick (block3)
        block3_pose = self.block3.get_pose().p
        arm_tag = ArmTag("left" if block3_pose[0] < 0 else "right")
        self.arm_tag = arm_tag

        # Pick and place in reverse order (3, 2, 1) so final order is large, medium, small left to right
        self._pick_and_place_block(self.block3, self.block3_target_pose, arm_tag)
        self._pick_and_place_block(self.block2, self.block2_target_pose, arm_tag)
        self._pick_and_place_block(self.block1, self.block1_target_pose, arm_tag)
    ...
    def _pick_and_place_block(self, block, target_pose, arm_tag):
    ...
    def check_success(self):
        ...
        return (np.all(np.abs(block1_pose[:2] - block2_pose[:2]) < eps)
                and np.all(np.abs(block2_pose[:2] - block3_pose[:2]) < eps)
