...
class place_object_basket(Base_Task_Multiview):
    """
    Single-arm: one arm grabs the object, puts it in the basket,
    then the same arm grabs the basket and moves it slightly.
    """
    ...
    def play_once(self):
        ...
        place_pose = np.array(place_pose, dtype=float)
        place_pose[:2] = f0[:2] if np.array_equal(place_pose[:2], f0[:2]) else f1[:2]
        ...
        # 3. Same arm grasps basket and moves it slightly
        self.move(self.grasp_actor(self.basket, arm_tag=self.arm_tag, pre_grasp_dis=0.08))
        self.move(
            self.move_by_displacement(
                arm_tag=self.arm_tag,
                x=0.05 if self.arm_tag == "right" else -0.05,
                z=0.05,
            ))
